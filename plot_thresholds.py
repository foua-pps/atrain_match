...
isACPGv2012=False
ROOT_DIR = "/local_disk/nina_pps/data_validation_ctth_patch_nov2012/VALIDATION_20130908_TEST1B_test/Reshaped_Files/npp/1km/"
...
isPPSCloudyOrClear = caObj.avhrr.all_arrays['cloudtype']>0
...
if isACPGv2012:
     (no_qflag, isPPSNight, twilight_flag, isPPSDay, all_dnt_flag) = get_day_night_twilight_info_pps2012(cloudtype_qflag) 
     (no_qflag, land_flag, isPPSSea, coast_flag, all_lsc_flag) = get_land_coast_sea_info_pps2012(cloudtype_qflag)
     isPPSIce = get_ice_info_pps2012(cloudtype_qflag)
else:
    (no_qflag, isPPSNight, twilight_flag, isPPSDay, all_dnt_flag) = get_day_night_twilight_info_pps2014(cloudtype_conditions)
    (no_qflag, land_flag, isPPSSea, coast_flag, all_lsc_flag) = get_land_coast_sea_info_pps2014(cloudtype_conditions)
...
isThinCirrus = np.logical_and(np.logical_and(np.logical_and(np.logical_and(isCloudy, isSingleLayerCloud), SeesThrough),isHigh),isOpticallyThin)
...
isWater = np.logical_and(isSea, isPPSSeaNotIce)
...
def plot_inner(args,isCloudy, isClear, TestOk, THRESHOLD):
    ...
    #isCloudyPPS = np.logical_and(isCloudy,isCloudyPPSALL)
...
def print_stats(cloudtype,isCloudyWaterNight,isClearWaterNight,isCloudyPPS,TestOkAll):
    print "Number of cloudy pixels sea night %d"%(
        len(cloudtype[isCloudyWaterNight==True]))
    print "Number of clear pixels sea night %d"%(
        len(cloudtype[isClearWaterNight==True]))
    detected_so_far=np.logical_and(TestOkAll, np.logical_and(
            isCloudyWaterNight,isCloudyPPS))
    would_have_been_detected_so_far = np.logical_and(TestOkAll, np.logical_and(
            isCloudyWaterNight,isPPSCloudyOrClear))
    would_have_been_missclassed_so_far = np.logical_and(TestOkAll, np.logical_and(
            isClearWaterNight,isPPSCloudyOrClear))
    print "POD cloudy sea night detected so far %f"%(
        len(cloudtype[detected_so_far==True])*1.0/
        len(cloudtype[isCloudyWaterNight==True]))
    false_so_far=np.logical_and(TestOkAll, np.logical_and(isClearWaterNight,isCloudyPPS))
    print "FAR cloudy so far %f"%(
        len(cloudtype[false_so_far==True])*1.0/len(cloudtype[detected_so_far==True]))
    print "POD cloudy sea night detected so far including extra test and new thresholds%f"%(
        len(cloudtype[would_have_been_detected_so_far==True])*1.0/
        len(cloudtype[isCloudyWaterNight==True]))
    false_so_far=np.logical_and(TestOkAll, np.logical_and(isClearWaterNight,isCloudyPPS))
    print "FAR cloudy so far including extra test and new thresholds %f"%(
        len(cloudtype[would_have_been_missclassed_so_far==True])*1.0/len(cloudtype[detected_so_far==True]))


print "Number of cloudy pixels sea night %d"%(len(cloudtype[isCloudyWaterNight==True]))
print "Number of clear pixels sea night %d"%(len(cloudtype[isClearWaterNight==True]))
print "Part of clouds sea night detected %f"%(sum(np.logical_and(isCloudyWaterNight,isCloudyPPS))*1.0/len(cloudtype[isCloudyWaterNight==True]))
print "Part of clear sea night missclassified %f"%(sum(np.logical_and(isClearWaterNight,isCloudyPPS))*1.0/len(cloudtype[isCloudyWaterNight==True]))
...
plot_test(args,isCloudyCirrusWaterNight,isClearWaterNight,TestOk,THRESHOLD)
args = {'title': "thinCirrusPrimaryTest_All_SeaNightNoIce_abslat",
        'xlable': 'abs Latitude',
        'ylable': 'T37-T12 minus dynamic threshold',
...
args = {'title': "arcticWarmCloudTest_All_SeaNight",
      'xlable': 't37text',
...
print t37text
THRESHOLD2 = OFFSETS['T37TEXT_OFFSET_SEA_NIGHT_ARCTIC'] + OFFSETS['QUALITY_MARGIN_T11TSUR_ARCTIC_INV'] #-15.0-1.0
THRESHOLD1 = 0.0 + -1*OFFSETS['QUALITY_MARGIN_T37TEXT']
TSUR_THRESHOLD = 0.0
...
# SEA DAY
####################################
print "Number of cloudy pixels sea day %d"%(len(cloudtype[isCloudyWaterDay==True]))
print "Number of clear pixels sea day %d"%(len(cloudtype[isClearWaterDay==True]))
print "Part of clouds sea day detected %f"%(sum(np.logical_and(isCloudyWaterDay,isCloudyPPS))*1.0/len(cloudtype[isCloudyWaterDay==True]))
print "Part of clear sea day missclassified %f"%(sum(np.logical_and(isClearWaterDay,isCloudyPPS))*1.0/len(cloudtype[isCloudyWaterDay==True]))
