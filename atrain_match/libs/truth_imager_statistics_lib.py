...
def get_subset_for_mode(match_obj, mode):
    ...
    if match_obj.truth_sat.lower() in ['calipso']:
        if getattr(match_obj_truth_sat, 'minimum_laser_energy_532', None) is not None:
...
def get_lwp_diff_inner_cloudsat(match_obj, val_subset, wide_selection=False):
    ...
    selection = np.logical_and(match_obj.imager.cpp_lwp >= 0,
                               match_obj.cloudsat.liq_water_path >= 0)
    ...
    if wide_selection:
        ...
        if getattr(match_obj.cloudsat, "CWC_status", None) is not None:
    ...
    lwp_diff = match_obj.imager.cpp_lwp - match_obj.cloudsat.liq_water_path
    lwp_diff = lwp_diff[selection]

    if getattr(match_obj.cloudsat, "LO_liquid_water_path", None) is not None:
        selection1 = np.logical_and(match_obj.imager.cpp_lwp >= 0,
                                    match_obj.cloudsat.LO_liquid_water_path >= 0)
        ...
        lwp_diff_lo = match_obj.imager.cpp_lwp - match_obj.cloudsat.LO_liquid_water_path
        lwp_diff_lo = lwp_diff_lo[selection1]
    else:
        lwp_diff_lo = []
    return lwp_diff, lwp_diff_lo, match_obj.imager.cpp_lwp, match_obj.cloudsat.liq_water_path, selection
...
def calculate_statistics(mode, statfilename, match_calipso, match_clsat, match_iss, match_amsr, match_synop,
    ...
    if match_clsat is not None:
        ...
        if val_subset is not None:
            ...
            if match_clsat.cloudsat.all_arrays['liq_water_path'] is not None:
