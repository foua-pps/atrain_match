...
try:
    from matchobject_io import read_truth_imager_match_obj, write_truth_imager_match_obj
except:
    from atrain_match.matchobject_io import (
...
def remove_doubles(mObj, mObj2):
    non_doubles = [
        ind
        for ind in range(len(mObj.calipso.sec_1970))
    ...
    if MAKE_EXTRA_CHECK:
        # Takes a lot of time ....
        doubles_time_and_id = [
            (mObj.calipso.sec_1970[ind], mObj.calipso.profile_id[ind, 0])
            for ind in range(len(mObj.calipso.sec_1970))
            if ind not in non_doubles
        ]

        extra_check = [
            time_and_id
            for time_and_id in doubles_time_and_id
            if time_and_id
            in zip(mObj2.calipso.sec_1970, mObj2.calipso.profile_id[:, 0])
        ]
        if len(extra_check) != len(mObj.calipso.sec_1970) - len(non_doubles):
            print(
                "Some points identified as doubles does not have "
    ...
    print(
        "Found {:d} doubles (out of {:d}) in the new object".format(
        ...
        for year in YEAR_LIST:
            for month in [
                ...
                for day in [f"{day:02d}" for day in range(32)]:
                    ...
                    if num_n > 0:
                        instrument = os.path.basename(filename).split("_")[-2]
                        if instrument not in ["avhrr", "seviri", "modis"]:
                            print("Unknown instrument - %s" % instrument)
                            pdb.set_trace()
                        outfile_template = "1km_{satellite}_%s%s%s_0000_99999_{truth}_{instrument}_match.h5".format(
                            satellite=satellite, truth=truth, instrument=instrument
                        )
                        filename_merged = outfile_template % (year, month, day)
                        ...
                        match_calipso_merged = None
                        print(
                            "File with all doublets %s files"
                            % len(files_all_doubles_month)
                        )
