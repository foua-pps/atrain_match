...
def do_the_printing(aObj, name):
    ...
    for use_t, surf in zip(use_surfs, surf_names):
        ...
        for condition in ["use", "no_cloudsat_iwp", "no_cloudsat_precip", "geoprof_cloudy", "without_zeros"]:
            ...
            plt.savefig(ADIR + "/PICTURES_FROM_PYTHON/VAL_2018_PLOTS/scatter_lwp_%s_%s_dist_all_cpp_%s_%s.png" %
                        (aObj.truth_sat, name, surf, condition), bbox_inches='tight')

            fig = plt.figure(figsize=(14, 4.5))
            ax = fig.add_subplot(131)
            hist_heights, x_, hist_heights_gaussian = my_hist(
                x[use_i] - y[use_i], None, bmin=-3000, bmax=3000, delta_h=5, return_also_corresponding_gaussian=True)
            ...
            # plt.show()
            plt.savefig(ADIR + "/PICTURES_FROM_PYTHON/VAL_2018_PLOTS/scatter_and_error_lwp_%s_%s_dist_all_cpp_170_%s_%s.png" %
                        (aObj.truth_sat, name, surf, condition), bbox_inches='tight')
            # plt.show()
            plt.close('all')
    ...
    plot_satellite_trajectory(lon[use_map], lat[use_map],
                              ADIR +
                              "/PICTURES_FROM_PYTHON/VAL_2018_PLOTS/map_marble_lwp_%s_%s_dist" % (aObj.truth_sat, name),
                              config.AREA_CONFIG_FILE,
                              fig_type=['png'])
    fig = plt.figure()
    ax = fig.add_subplot(111)
    ...
    distribution_map(lon[use_map],
                     lat[use_map])
    plt.savefig(ADIR + "/PICTURES_FROM_PYTHON/VAL_2018_PLOTS/map_white_lwp_%s_%s_dist_all_cpp.png" %
                (aObj.truth_sat, name), bbox_inches='tight')


if __name__ == "__main__":
    ROOT_DIR_v2014_amsr = (
        ADIR + "/DATA_MISC/reshaped_files_validation_2018/global_gac_v2014_created20180927/Reshaped_Files_merged_amsr_lwp/noaa18/5km/")
    ROOT_DIR_v2018_amsr = (
        ADIR + "/DATA_MISC/reshaped_files_validation_2018/global_gac_v2018_created20180927/Reshaped_Files_merged_amsr_lwp/noaa18/5km/")
    ...
    ROOT_DIR_v2018_modis_cl = (
        ADIR + "/DATA_MISC/reshaped_files_validation_2018/global_modis_v2018_created20181001_cmap_osiice_dust/Reshaped_Files_merged_cloudsat_lwp/eos2/1km/*/*/")
    ROOT_DIR_v2018_modis_amsr = (
        ADIR + "/DATA_MISC/reshaped_files_validation_2018/global_modis_v2018_created20181001_cmap_osiice_dust/Reshaped_Files_merged_amsr_lwp/eos2/1km/*/*/")
    ROOT_DIR_v2014_modis_amsr = (
        ADIR + "/DATA_MISC/reshaped_files_validation_2018/global_modis_v2014_created20180920/Reshaped_Files_merged_amsr_lwp/eos2/1km/*/*/")
