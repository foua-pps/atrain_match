...
class ppsMatch_Imager_CalipsoObject(DataObject):
    ...
    def set_false_and_missed_cloudy_and_clear(self, caObj):
        ...
        if caObj.avhrr.all_arrays['cloudtype'] is not None:
            ...
            isCloudyPPS = np.logical_and(caObj.avhrr.all_arrays['cloudmask']>0,
        ...
        if self.cc_method == 'BASIC' and self.isGAC:
            ...
            #Let's try to correct for the "artificial" jump in CFC at cot value 1.0 due to added 1 km CALIPSO clouds with cot set to 1.0. These clouds should be moved to the other extreme at cot = 5.0 (they are definitely bright clouds!)
            isArtificialCloudy = np.logical_and(
                caObj.calipso.all_arrays['cloud_fraction']<1.0,
                caObj.calipso.all_arrays['total_optical_depth_5km']==1.0)
            isArtificialCloudy = np.logical_and(
                isArtificialCloudy,
                caObj.calipso.all_arrays['cloud_fraction']>0.5)
                                
            if filtcot > 1.0:
                #("Artificial" jump in CFC due to added 1 km CALIPSO clouds with tau=1.0 ==> remove jump and add these cases to category 5.0!
                isCalipsoCloudy = np.logical_and(
                    caObj.calipso.all_arrays['cloud_fraction']>0.5,
                    caObj.calipso.all_arrays['total_optical_depth_5km']>=filtcot)
                isCalipsoCloudy = np.logical_or(isCalipsoCloudy,isArtificialCloudy)
                isCalipsoClear = np.not_equal(isCalipsoCloudy, True)
            else:            
                isCalipsoCloudy = np.logical_and(
                    caObj.calipso.all_arrays['cloud_fraction']>0.5,
                    #caObj.calipso.all_arrays['total_optical_depth_5km']>filtcot) #This small error was actually serious!!!!
                    caObj.calipso.all_arrays['total_optical_depth_5km']>=filtcot)
                #caObj.calipso.all_arrays['total_optical_depth_5km']>0.225)
                #Changed COT threshold from 0.15 to 0.225 when going to CALIPSO version 4.10/KG 2007-04-20
                isCalipsoClear = np.not_equal(isCalipsoCloudy, True)
        ...
        if self.DNT in ["day"]:
            ...
            isCloudyPPS = np.logical_and(isCloudyPPS,  sunz<=0.80) #Prevous bug CLARA-A2
            isClearPPS =  np.logical_and(isClearPPS,  sunz<=0.80)  #Prevous bug CLARA-A
        if self.DNT in ["night"]:
            #isCloudyPPS = np.logical_and(isCloudyPPS,  sunz>=95)
            #isClearPPS =  np.logical_and(isClearPPS,  sunz>=95)
            isCloudyPPS = np.logical_and(isCloudyPPS,  sunz>=0.95) #Prevous bug CLARA-A
            isClearPPS =  np.logical_and(isClearPPS,  sunz>=0.95)  #Prevous bug CLARA-A
...
##             plot_area_name)[0]
    ...
    def calculate_calipso_cfc(self):
        ...
        self.find_number_of_clouds_clear()
        #pdb.set_trace()
##         total_cases = np.sum(self.N)
##         print "Total number of observations: ", total_cases
        calipso_cfc = 100*(
...
def write_filtered_TCC_POD_without_FAR_without_N(filename, TCC, POD, lat, lon, radius_km):
    """
    ...
    with h5py.File(filename, 'w') as f:
        f.create_dataset('calipso_cloud_cover', data=TCC,
                         compression=COMPRESS_LVL)
        f.create_dataset('pod_cloudy', data=POD,
                         compression=COMPRESS_LVL)
...
def write_filtered_TCC_POD_with_FAR_with_N(filename, TCC, POD, FAR, N, lat, lon, radius_km):
    """
    Write match objects to HDF5 file *filename*.
    ...
    with h5py.File(filename, 'w') as f:
        ...
        f.create_dataset('pod_cloudy', data=POD,
                         compression=COMPRESS_LVL)
        f.create_dataset('far_cloudy', data=FAR,
                         compression=COMPRESS_LVL)
        f.create_dataset('total_cases', data=N,
                         compression=COMPRESS_LVL)
        f.create_dataset('latitude', data=lat,
                         compression=COMPRESS_LVL)
        f.create_dataset('longitude', data=lon,
...
def read_filtered_TCC_POD_with_FAR_with_N(filename):
    import h5py
    h5file = h5py.File(filename, 'r')
    TCC = h5file['calipso_cloud_cover'].value
    POD = h5file['pod_cloudy'].value
    FAR = h5file['far_cloudy'].value
    tot_cases = h5file['total_cases'].value
    lat = h5file['latitude'].value
    lon = h5file['longitude'].value
    radius_km = h5file['fibonacci_radius_km'].value

    h5file.close()

    return TCC, POD, FAR, tot_cases, lat, lon, radius_km

def read_filtered_TCC_POD_without_FAR_without_N(filename):
    import h5py
    h5file = h5py.File(filename, 'r')
    TCC = h5file['calipso_cloud_cover'].value
    POD = h5file['pod_cloudy'].value
    ...
    return TCC, POD, lat, lon, radius_km


def write_POD_CLOUDY_LAYER(pod_cloudy_layer_filename,POD_CLOUDY_LAYER, lat, lon, radius_km):

    """
    Write probability of detection for cot layers to HDF5 file *filename*.
    """
    ...
    with h5py.File(pod_cloudy_layer_filename, 'w') as f:
        g = f.create_group('cot_layer_probabilities')
        g.create_dataset('0.025', data=POD_CLOUDY_LAYER['0_025'],
                                     compression=COMPRESS_LVL)
        g.create_dataset('0.075', data=POD_CLOUDY_LAYER['0_075'],
                                     compression=COMPRESS_LVL)
        g.create_dataset('0.125', data=POD_CLOUDY_LAYER['0_125'],
                                     compression=COMPRESS_LVL)
        g.create_dataset('0.175', data=POD_CLOUDY_LAYER['0_175'],
                                     compression=COMPRESS_LVL)
        g.create_dataset('0.225', data=POD_CLOUDY_LAYER['0_225'],
                                     compression=COMPRESS_LVL)
        g.create_dataset('0.275', data=POD_CLOUDY_LAYER['0_275'],
                                     compression=COMPRESS_LVL)
        g.create_dataset('0.325', data=POD_CLOUDY_LAYER['0_325'],
                                     compression=COMPRESS_LVL)
        g.create_dataset('0.375', data=POD_CLOUDY_LAYER['0_375'],
                                     compression=COMPRESS_LVL)
        g.create_dataset('0.425', data=POD_CLOUDY_LAYER['0_425'],
                                     compression=COMPRESS_LVL)
        g.create_dataset('0.475', data=POD_CLOUDY_LAYER['0_475'],
                                     compression=COMPRESS_LVL)
        g.create_dataset('0.55', data=POD_CLOUDY_LAYER['0_55'],
                                     compression=COMPRESS_LVL)
        g.create_dataset('0.65', data=POD_CLOUDY_LAYER['0_65'],
                                     compression=COMPRESS_LVL)
        g.create_dataset('0.75', data=POD_CLOUDY_LAYER['0_75'],
                                     compression=COMPRESS_LVL)
        g.create_dataset('0.85', data=POD_CLOUDY_LAYER['0_85'],
                                     compression=COMPRESS_LVL)
        g.create_dataset('0.95', data=POD_CLOUDY_LAYER['0_95'],
                                     compression=COMPRESS_LVL)
        g.create_dataset('1.50', data=POD_CLOUDY_LAYER['1_50'],
                                     compression=COMPRESS_LVL)
        g.create_dataset('2.50', data=POD_CLOUDY_LAYER['2_50'],
                                     compression=COMPRESS_LVL)
        g.create_dataset('3.50', data=POD_CLOUDY_LAYER['3_50'],
                                     compression=COMPRESS_LVL)
        g.create_dataset('4.50', data=POD_CLOUDY_LAYER['4_50'],
                                     compression=COMPRESS_LVL)
        f.create_dataset('latitude', data=lat,
                         compression=COMPRESS_LVL)
        f.create_dataset('longitude', data=lon,
...
def read_POD_CLOUDY_LAYER_afternoon(pod_cloudy_layer_filename):

    """
    Read probability of detection for cot layers in HDF5 file *filename*.
    """
    ...
    h5file = h5py.File(pod_cloudy_layer_filename, 'r')
    
    POD_CLOUDY_LAYER_AFTERNOON['0_025'] = h5file['cot_layer_probabilities/0.025'].value
    POD_CLOUDY_LAYER_AFTERNOON['0_075'] = h5file['cot_layer_probabilities/0.075'].value
    POD_CLOUDY_LAYER_AFTERNOON['0_125'] = h5file['cot_layer_probabilities/0.125'].value
    POD_CLOUDY_LAYER_AFTERNOON['0_175'] = h5file['cot_layer_probabilities/0.175'].value
    POD_CLOUDY_LAYER_AFTERNOON['0_225'] = h5file['cot_layer_probabilities/0.225'].value
    POD_CLOUDY_LAYER_AFTERNOON['0_275'] = h5file['cot_layer_probabilities/0.275'].value
    POD_CLOUDY_LAYER_AFTERNOON['0_325'] = h5file['cot_layer_probabilities/0.325'].value
    POD_CLOUDY_LAYER_AFTERNOON['0_375'] = h5file['cot_layer_probabilities/0.375'].value
    POD_CLOUDY_LAYER_AFTERNOON['0_425'] = h5file['cot_layer_probabilities/0.425'].value
    POD_CLOUDY_LAYER_AFTERNOON['0_475'] = h5file['cot_layer_probabilities/0.475'].value
    POD_CLOUDY_LAYER_AFTERNOON['0_55'] = h5file['cot_layer_probabilities/0.55'].value
    POD_CLOUDY_LAYER_AFTERNOON['0_65'] = h5file['cot_layer_probabilities/0.65'].value
    POD_CLOUDY_LAYER_AFTERNOON['0_75'] = h5file['cot_layer_probabilities/0.75'].value
    POD_CLOUDY_LAYER_AFTERNOON['0_85'] = h5file['cot_layer_probabilities/0.85'].value
    POD_CLOUDY_LAYER_AFTERNOON['0_95'] = h5file['cot_layer_probabilities/0.95'].value
    POD_CLOUDY_LAYER_AFTERNOON['1_50'] = h5file['cot_layer_probabilities/1.50'].value
    POD_CLOUDY_LAYER_AFTERNOON['2_50'] = h5file['cot_layer_probabilities/2.50'].value
    POD_CLOUDY_LAYER_AFTERNOON['3_50'] = h5file['cot_layer_probabilities/3.50'].value
    POD_CLOUDY_LAYER_AFTERNOON['4_50'] = h5file['cot_layer_probabilities/4.50'].value

    lats = h5file['latitude'].value
    lons = h5file['longitude'].value
    radius_km = h5file['fibonacci_radius_km'].value
    h5file.close()

    return POD_CLOUDY_LAYER_AFTERNOON, lats, lons, radius_km

def read_POD_CLOUDY_LAYER_morning(pod_cloudy_layer_filename):
    ...
    POD_CLOUDY_LAYER_MORNING['0_025'] = h5file['cot_layer_probabilities/0.025'].value
    POD_CLOUDY_LAYER_MORNING['0_075'] = h5file['cot_layer_probabilities/0.075'].value
    POD_CLOUDY_LAYER_MORNING['0_125'] = h5file['cot_layer_probabilities/0.125'].value
    POD_CLOUDY_LAYER_MORNING['0_175'] = h5file['cot_layer_probabilities/0.175'].value
    POD_CLOUDY_LAYER_MORNING['0_225'] = h5file['cot_layer_probabilities/0.225'].value
    POD_CLOUDY_LAYER_MORNING['0_275'] = h5file['cot_layer_probabilities/0.275'].value
    POD_CLOUDY_LAYER_MORNING['0_325'] = h5file['cot_layer_probabilities/0.325'].value
    POD_CLOUDY_LAYER_MORNING['0_375'] = h5file['cot_layer_probabilities/0.375'].value
    POD_CLOUDY_LAYER_MORNING['0_425'] = h5file['cot_layer_probabilities/0.425'].value
    POD_CLOUDY_LAYER_MORNING['0_475'] = h5file['cot_layer_probabilities/0.475'].value
    POD_CLOUDY_LAYER_MORNING['0_55'] = h5file['cot_layer_probabilities/0.55'].value
    POD_CLOUDY_LAYER_MORNING['0_65'] = h5file['cot_layer_probabilities/0.65'].value
    POD_CLOUDY_LAYER_MORNING['0_75'] = h5file['cot_layer_probabilities/0.75'].value
    POD_CLOUDY_LAYER_MORNING['0_85'] = h5file['cot_layer_probabilities/0.85'].value
    POD_CLOUDY_LAYER_MORNING['0_95'] = h5file['cot_layer_probabilities/0.95'].value
    POD_CLOUDY_LAYER_MORNING['1_50'] = h5file['cot_layer_probabilities/1.50'].value
    POD_CLOUDY_LAYER_MORNING['2_50'] = h5file['cot_layer_probabilities/2.50'].value
    POD_CLOUDY_LAYER_MORNING['3_50'] = h5file['cot_layer_probabilities/3.50'].value
    POD_CLOUDY_LAYER_MORNING['4_50'] = h5file['cot_layer_probabilities/4.50'].value
    ...
    return POD_CLOUDY_LAYER_MORNING, lats, lons, radius_km
