...
def my_measures(calipso_cfc, pps_cfc, calipso_snowi, pps_snowi, thin, use):
    ...
    indict["pod_cloudy"] = (indict["pps_cloudy_c_cloudy_w"]+indict["pps_cloudy_c_cloudy_i"]) * \
        100.0/(np.sum(c_cloudy_w) + np.sum(c_cloudy_i))
    indict["pod_water"] = (indict["pps_water_c_water"])*100.0/(np.sum(c_water))
    indict["pod_ice"] = (indict["pps_ice_c_ice"])*100.0/(np.sum(c_ice))
    indict["far_cloudy"] = (indict["pps_cloudy_c_ice"]+indict["pps_cloudy_c_water"])*100.0/(
        indict["pps_cloudy_c_cloudy_w"]+indict["pps_cloudy_c_cloudy_i"] + indict["pps_cloudy_c_ice"]+indict["pps_cloudy_c_water"])
    indict["far_water"] = (indict["pps_water_c_cloudy_i"]+indict["pps_water_c_cloudy_w"])*100.0/(
        indict["pps_water_c_cloudy_w"]+indict["pps_water_c_cloudy_i"] + indict["pps_water_c_ice"]+indict["pps_water_c_water"])
    indict["far_ice"] = (indict["pps_ice_c_cloudy_i"]+indict["pps_ice_c_cloudy_w"])*100.0 / \
        (indict["pps_ice_c_cloudy_w"]+indict["pps_ice_c_cloudy_i"] + indict["pps_ice_c_ice"]+indict["pps_ice_c_water"])
...
def plot_cfc_table(match_calipso, cfc_limit=0.9, sat="modis"):
    ...
    (no_qflag, land_flag, sea_flag, coast_flag, all_lsc_flag) = get_land_coast_sea_info_pps2014(
        match_calipso.imager.cloudtype_conditions)
