...
def replot_figure2_from_saved_data(month):
    ...
    def plot_one(ax, x_data, y_data, sat, cloudc, legend=False):
        ...
        if legend:
            pass
            plt.legend(fancybox=True, loc=1, numpoints=3, bbox_to_anchor=(
                1.25, 1.1), framealpha=1.0, edgecolor='w', fontsize=18)
        ax.grid(True)
        plt.text(0.024, 0.90, text_i, fontsize=18, transform=ax.transAxes,
...
def plot_medium_bias_plot_from_saved_data(month):
    ...
    def plot_one(ax, x_data, y_data, median_data, bias_data, std_data, sat, cloudc, imager, text_i, maxx, maxy, legend=False):
        ...
        plt.plot([median_data[sat][cloudc][imager], median_data[sat][cloudc]
                  [imager]], [0, ylim_median], color3, label="median")
        ...
        if legend:
            pass
            plt.legend(fancybox=True, loc=1, numpoints=3, bbox_to_anchor=(
                1.30, 1.1), framealpha=1.0, edgecolor='w', fontsize=18)
        ax.grid(True)
        plt.text(0.025, 0.90, text_i, fontsize=18, transform=ax.transAxes,
                 bbox=dict(facecolor='w', edgecolor='w', alpha=1.0))
    ...
    for cloudc, maxx, maxy, in zip(["All", "Low", "Medium", "High"],
        ...
        if cloudc not in ["All"]:
            plt.suptitle(cloudc)
        ax = fig.add_subplot(321)
        plot_one(ax, x_data, y_data, median_data, bias_data, std_data,
                 "cloudsat", cloudc, "PPS-v2014", " (a) ", maxx, maxy)  # , True)
        ax = fig.add_subplot(323)
        plot_one(ax, x_data, y_data, median_data, bias_data, std_data,
                 "cloudsat", cloudc, "MODIS-C6", " (c) ", maxx, maxy)  # , True)
        ax = fig.add_subplot(325)
        plot_one(ax, x_data, y_data, median_data, bias_data, std_data,
                 "cloudsat", cloudc, "NN-AVHRR", " (e) ", maxx, maxy)  # , True)
        ...
        ax = fig.add_subplot(322)
        plot_one(ax, x_data, y_data, median_data, bias_data, std_data,
                 "calipso", cloudc, "PPS-v2014", " (b) ", maxx, maxy, True)
        ax = fig.add_subplot(324)
        plot_one(ax, x_data, y_data, median_data, bias_data, std_data,
                 "calipso", cloudc, "MODIS-C6", " (d) ", maxx, maxy, True)
        ax = fig.add_subplot(326)
        plot_one(ax, x_data, y_data, median_data, bias_data, std_data,
                 "calipso", cloudc, "NN-AVHRR", " (f) ", maxx, maxy, True)

        plt.savefig(ADIR + "/PICTURES_FROM_PYTHON/CTTH_PLOTS/fig08_bias_median_%s_%s.pdf" %
                    (cloudc, month), bbox_inches='tight')
        plt.close("all")
        fig = plt.figure(figsize=(9, 11))
        ax = fig.add_subplot(111)
        plot_one(ax, x_data, y_data, median_data, bias_data, std_data,
                 "cloudsat", cloudc, "NN-AVHRR", " (e) ", maxx, maxy, True)
        plt.savefig(ADIR + "/PICTURES_FROM_PYTHON/CTTH_PLOTS/fig08_bias_median_%s_%s_e.pdf" %
                    (cloudc, month), bbox_inches='tight')
...
def investigate_nn_ctth_modis_lvl2_cloudsat():
    ...
    name = ""
    # [ "03", "05", "07", "09", "11"]: #[ "02", "04", "06", "08", "10", "12"]:# , "03", "05", "07", "09", "11", "01" ]:  #[ "06", "09"]:
