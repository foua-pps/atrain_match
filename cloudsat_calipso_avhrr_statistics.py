...
def CalculateStatistics(ICE_COVER_SEA,ICE_FREE_SEA,SNOW_COVER_LAND,SNOW_FREE_LAND,COASTAL_ZONE,clsatObj,CLOUDSAT_CLOUDY_THR,statfile,EMISS_FILTERING,caObj\
                        ,cal_MODIS_cflag,cal_vert_feature,PLOT_OPTION,MAXHEIGHT,avhrr_ctth_csat_ok,data_ok,cal_data_ok,avhrr_ctth_cal_ok,caliop_max_height\
...
### 1km Data ###      
    ...
    ##         cal_subset = numpy.logical_and(numpy.logical_and(numpy.less(caObj.calipso.nsidc,100),numpy.greater(caObj.calipso.nsidc,10)),numpy.equal(caObj.calipso.igbp,17))
        ...
        cloudsat_cloud_mask=numpy.greater_equal(cloudsat_cloud_mask,CLOUDSAT_CLOUDY_THR)
...
### 5km Data ###----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    ##         cal_subset = numpy.logical_and(numpy.logical_and(numpy.less(caObj.calipso5km.nsidc,100),numpy.greater(caObj.calipso5km.nsidc,10)),numpy.equal(caObj.calipso5km.igbp,17))
            # Unfortunately, the above formulation used for ORR-B excluded the case when ice-cover was exactly 100 %!!! Very embarrassing!/KG
            cal_subset = numpy.logical_and(numpy.logical_and(numpy.less_equal(caObj.calipso5km.nsidc,100),numpy.greater(caObj.calipso5km.nsidc,10)),numpy.equal(caObj.calipso5km.igbp,17))
        elif ICE_FREE_SEA:
            cal_subset = numpy.logical_and(numpy.equal(caObj.calipso5km.nsidc,0),numpy.equal(caObj.calipso5km.igbp,17))
        elif SNOW_COVER_LAND:
            cal_subset = numpy.logical_and(numpy.logical_and(numpy.less(caObj.calipso5km.nsidc,104),numpy.greater(caObj.calipso5km.nsidc,10)),numpy.not_equal(caObj.calipso5km.igbp,17))
            # Notice that some uncertainty remains about the meaning of IGBP category 15 = "snow and ice". Can this possibly include also the Arctic ice sheet? We hope that it is not!!! However, if it is, the whole classification here might be wrong since this will affect also the definition of IGBP category 17./KG 
        elif SNOW_FREE_LAND:
            cal_subset = numpy.logical_and(numpy.equal(caObj.calipso5km.nsidc,0),numpy.not_equal(caObj.calipso5km.igbp,17))
        elif COASTAL_ZONE:
            cal_subset = numpy.equal(caObj.calipso5km.nsidc,255)
        ...
        cloudsat_clear =  numpy.less(cloudsat_cloud_fraction,1)
        cloudsat_cloudy = numpy.greater_equal(cloudsat_cloud_fraction,1)
        pps_clear = numpy.logical_and(numpy.less_equal(clsatObj.avhrr5km.cloudtype,4),numpy.greater(clsatObj.avhrr5km.cloudtype,0))
        pps_cloudy = numpy.logical_and(numpy.greater(clsatObj.avhrr5km.cloudtype,4),numpy.less(clsatObj.avhrr5km.cloudtype,20))
     ...
     # CORRELATION CLOUD MASK: CLOUDSAT - MODIS
        ...
        modis_clear = numpy.logical_or(numpy.equal(clsatObj.cloudsat5km.MODIS_cloud_flag,1),
                                          numpy.equal(clsatObj.cloudsat5km.MODIS_cloud_flag,0))
        modis_cloudy = numpy.logical_or(numpy.equal(clsatObj.cloudsat5km.MODIS_cloud_flag,3),
                                          numpy.equal(clsatObj.cloudsat5km.MODIS_cloud_flag,2))
        ...
        if EMISS_FILTERING:
            calipso_clear = numpy.logical_and(numpy.less(caObj.calipso5km.cloud_fraction,0.34),emissfilt_calipso_ok)
            calipso_cloudy = numpy.logical_and(numpy.greater(caObj.calipso5km.cloud_fraction,0.66),emissfilt_calipso_ok)
            pps_clear = numpy.logical_and(numpy.less_equal(caObj.avhrr5km.cloudtype,4),emissfilt_calipso_ok)
            pps_cloudy = numpy.logical_and(numpy.greater(caObj.avhrr5km.cloudtype,4),emissfilt_calipso_ok)

            #print "------------------------------------"
            #print "STATISTICS CLOUD MASK: CALIOP - AVHRR - Emissivity filtered limit= ", EMISS_LIMIT
        elif ICE_COVER_SEA:
            calipso_clear = numpy.logical_and(numpy.less(caObj.calipso5km.cloud_fraction,0.34),cal_subset)
            calipso_cloudy = numpy.logical_and(numpy.greater(caObj.calipso5km.cloud_fraction,0.66),cal_subset)
            pps_clear = numpy.logical_and(numpy.less_equal(caObj.avhrr5km.cloudtype,4),cal_subset)
            pps_cloudy = numpy.logical_and(numpy.greater(caObj.avhrr5km.cloudtype,4),cal_subset)
            #print "------------------------------------"
            #print "STATISTICS CLOUD MASK: CALIOP - AVHRR - Exclusively over ocean ice:  "
        elif ICE_FREE_SEA:
            calipso_clear = numpy.logical_and(numpy.less(caObj.calipso5km.cloud_fraction,0.34),cal_subset)
            ...
            calipso_clear = numpy.less(caObj.calipso5km.cloud_fraction,0.34)
            calipso_cloudy = numpy.greater(caObj.calipso5km.cloud_fraction,0.66)
            pps_clear = numpy.logical_and(numpy.less_equal(caObj.avhrr5km.cloudtype,4),numpy.greater(caObj.avhrr5km.cloudtype,0))
            pps_cloudy = numpy.logical_and(numpy.greater(caObj.avhrr5km.cloudtype,4),numpy.less(caObj.avhrr5km.cloudtype,20))
            #print "------------------------------------"
            #print "STATISTICS CLOUD MASK: CALIOP - AVHRR"
        ...
        if EMISS_FILTERING:
            ...
            calipso_medium = numpy.logical_and(numpy.logical_and(numpy.greater(cal_vert_feature[::],3),numpy.less_equal(cal_vert_feature[::],5)),emissfilt_calipso_ok)
            calipso_high = numpy.logical_and(numpy.logical_and(numpy.greater_equal(cal_vert_feature[::],5),numpy.less_equal(cal_vert_feature[::],7)),emissfilt_calipso_ok)
            avhrr_low = numpy.logical_and(numpy.logical_and(numpy.greater_equal(caObj.avhrr5km.cloudtype,5),numpy.less_equal(caObj.avhrr5km.cloudtype,8)),emissfilt_calipso_ok)
            avhrr_medium = numpy.logical_and(numpy.logical_and(numpy.greater_equal(caObj.avhrr5km.cloudtype,9),numpy.less_equal(caObj.avhrr5km.cloudtype,10)),emissfilt_calipso_ok)
            avhrr_high = numpy.logical_and(numpy.logical_and(numpy.greater_equal(caObj.avhrr5km.cloudtype,11),numpy.less_equal(caObj.avhrr5km.cloudtype,18)),emissfilt_calipso_ok)
            avhrr_frac = numpy.logical_and(numpy.logical_and(numpy.greater_equal(caObj.avhrr5km.cloudtype,19),numpy.less_equal(caObj.avhrr5km.cloudtype,19)),emissfilt_calipso_ok)
            calipso_clear = numpy.logical_and(numpy.less(caObj.calipso5km.cloud_fraction,0.34),emissfilt_calipso_ok)
            avhrr_clear = numpy.logical_and(numpy.logical_and(numpy.less_equal(caObj.avhrr5km.cloudtype,4),numpy.greater(caObj.avhrr5km.cloudtype,0)),emissfilt_calipso_ok)
            #print "STATISTICS CLOUD TYPE: CALIOP - AVHRR - Emissivity filtered limit= ", EMISS_LIMIT
        elif ICE_COVER_SEA:
            calipso_low = numpy.logical_and(numpy.logical_and(numpy.greater_equal(cal_vert_feature[::],0),numpy.less_equal(cal_vert_feature[::],3)),cal_subset)
            calipso_medium = numpy.logical_and(numpy.logical_and(numpy.greater(cal_vert_feature[::],3),numpy.less_equal(cal_vert_feature[::],5)),cal_subset)
            calipso_high = numpy.logical_and(numpy.logical_and(numpy.greater_equal(cal_vert_feature[::],5),numpy.less_equal(cal_vert_feature[::],7)),cal_subset)
            avhrr_low = numpy.logical_and(numpy.logical_and(numpy.greater_equal(caObj.avhrr5km.cloudtype,5),numpy.less_equal(caObj.avhrr5km.cloudtype,8)),cal_subset)
            avhrr_medium = numpy.logical_and(numpy.logical_and(numpy.greater_equal(caObj.avhrr5km.cloudtype,9),numpy.less_equal(caObj.avhrr5km.cloudtype,10)),cal_subset)
            avhrr_high = numpy.logical_and(numpy.logical_and(numpy.greater_equal(caObj.avhrr5km.cloudtype,11),numpy.less_equal(caObj.avhrr5km.cloudtype,18)),cal_subset)
            avhrr_frac = numpy.logical_and(numpy.logical_and(numpy.greater_equal(caObj.avhrr5km.cloudtype,19),numpy.less_equal(caObj.avhrr5km.cloudtype,19)),cal_subset)
            calipso_clear = numpy.logical_and(numpy.less(caObj.calipso5km.cloud_fraction,0.34),cal_subset)
            avhrr_clear = numpy.logical_and(numpy.logical_and(numpy.less_equal(caObj.avhrr5km.cloudtype,4),numpy.greater(caObj.avhrr5km.cloudtype,0)),cal_subset)
            #print "------------------------------------"
            #print "STATISTICS CLOUD TYPE: CALIOP - AVHRR - Exclusively over ocean ice:  "
        elif ICE_FREE_SEA:
            calipso_low = numpy.logical_and(numpy.logical_and(numpy.greater_equal(cal_vert_feature[::],0),numpy.less_equal(cal_vert_feature[::],3)),cal_subset)
            ...
            calipso_medium = numpy.logical_and(numpy.greater(cal_vert_feature[::],3),numpy.less_equal(cal_vert_feature[::],5))
            calipso_high = numpy.logical_and(numpy.greater_equal(cal_vert_feature[::],5),numpy.less_equal(cal_vert_feature[::],7))
            avhrr_low = numpy.logical_and(numpy.greater_equal(caObj.avhrr5km.cloudtype,5),numpy.less_equal(caObj.avhrr5km.cloudtype,8))
            avhrr_medium = numpy.logical_and(numpy.greater_equal(caObj.avhrr5km.cloudtype,9),numpy.less_equal(caObj.avhrr5km.cloudtype,10))
            avhrr_high = numpy.logical_and(numpy.greater_equal(caObj.avhrr5km.cloudtype,11),numpy.less_equal(caObj.avhrr5km.cloudtype,18))
            avhrr_frac = numpy.logical_and(numpy.greater_equal(caObj.avhrr5km.cloudtype,19),numpy.less_equal(caObj.avhrr5km.cloudtype,19))
            calipso_clear = numpy.less(caObj.calipso5km.cloud_fraction,0.34)
            avhrr_clear = numpy.logical_and(numpy.less_equal(caObj.avhrr5km.cloudtype,4),numpy.greater(caObj.avhrr5km.cloudtype,0))
        ...
        if not PLOT_OPTION:
            dummy=clsatObj.cloudsat5km.latitude.shape[0]
            pixel_position_plain=numpy.arange(dummy)
            clsat_max_height = numpy.repeat(clsatObj.cloudsat5km.Height[124,::],data_ok)
    #        clsat_max_height = numpy.zeros(dummy, 'f')
    ...
    #                if nidx >= 20:
        ...
        if len(avhrr_height) > 0:
            ...
            #print "Bias-corrected RMS difference cloudsat-avhrr = ",RMS_difference
            #print "Number of matchups: ", len(avhrr_height)
            #print "Number of failing PPS CTTHs: ", len(numpy.repeat(clsatObj.avhrr5km.cloudtype[::],numpy.greater(clsatObj.avhrr5km.cloudtype,4)))-len(avhrr_height)
            #print
        ...
        #print "Bias-corrected RMS difference caliop-avhrr = ",RMS_difference_biascorr
        #print "Number of matchups: ", len(avhrr_height_work)
        #print "Number of failing PPS CTTHs: ", len(numpy.repeat(caObj.avhrr5km.cloudtype[::],numpy.greater(caObj.avhrr5km.cloudtype,4)))-len(avhrr_height_work)
        statfile.write("CLOUD HEIGHT CALIOP ALL: %f %f %f %s %f \n" % (corr_caliop_avhrr,bias,RMS_difference,len(avhrr_height_work),sum(diff_squared_biascorr)))
        # THEN FOR LOW CLOUDS (VERTICAL FEATURE MASK CATEGORIES 0-3)
        cal_low_ok = numpy.logical_and(numpy.greater_equal(cal_vert_feature[::],0),numpy.less_equal(cal_vert_feature[::],3))
        cal_low_ok = numpy.logical_and(cal_low_ok,okcaliop)
