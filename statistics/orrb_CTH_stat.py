...
class CloudTopStats(OrrbStats):
    ...
    def printout(self):
        try:
            ...
            lines.append("Mean error: %.0f" % self.bias_csa)
            lines.append("RMS error: %.0f" % self.rms_csa)
            #lines.append("RMS error: %.0f" % self.rms_csa)
            lines.append("")
            ...
            lines.append("Mean error total cases: %.0f" % self.bias_cal_all)
            lines.append("Mean error low-level cases: %.0f" % self.bias_cal_low)
            lines.append("Mean error medium-level cases: %.0f" % self.bias_cal_medium)
            lines.append("Mean error high-level cases: %.0f" % self.bias_cal_high)
            lines.append("RMS error total cases: %.0f" % self.rms_cal_all)
            lines.append("RMS error low-level cases: %.0f" % self.rms_cal_low)
            lines.append("RMS error medium-level cases: %.0f" % self.rms_cal_medium)
            lines.append("RMS error high-level cases: %.0f" % self.rms_cal_high)
            lines.append("bc-RMS error total cases: %.0f" % self.bcrms_cal_all)
            lines.append("bc-RMS error low-level cases: %.0f" % self.bcrms_cal_low)
            lines.append("bc-RMS error medium-level cases: %.0f" % self.bcrms_cal_medium)
            lines.append("bc-RMS error high-level cases: %.0f" % self.bcrms_cal_high)
            lines.append("")
