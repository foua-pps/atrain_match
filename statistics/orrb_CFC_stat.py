...
class CloudFractionStats(OrrbStats):
    ...
    def printout(self):
        try:
            ...
            lines.append("Mean CFC Cloudsat: %.2f " % self.mean_CFC_csa)
            lines.append("Mean error: %.2f" % self.bias_csa_perc)
            lines.append("RMS error: %.2f" % self.rms_csa)
            lines.append("")
            lines.append("Total number of CALIOP matched FOVs: %d" % self.samples_cal)
            lines.append("Mean CFC CALIOP: %.2f " % self.mean_CFC_cal)
