...
DSEC_PER_AVHRR_SCALINE5KM = 1.0/6*5 # A "work for the time being" solution
...
# ----------------------------------------
def readCaliop5kmAvhrr5kmMatchObj(filename):
    import _pyhl
...
# ----------------------------------------
def writeCaliop5kmAvhrr5kmMatchObj(filename,ca_obj,compress_lvl):
    import _pyhl
    status = -1
...
# ------------------------------------------------------------------
def writeCoverage5km(covIn,filename,inAid,outAid):
    import _pyhl
...
# --------------------------------------------
def get_calipso5km_avhrr5km_linpix(avhrrIn,lon,lat):
    #import numpy.oldnumeric as Numeric
    import numpy
    tmppcs="tmpproj"
    ...
    while startline < avhrrIn.longitude.shape[0]:
        ...
        cal,cap,ok = get_calipso5km_avhrr5km_linpix_segment(avhrrIn,lon,lat,
                                                      (startline,endline),
...
# --------------------------------------------
def get_calipso5km_avhrr5km_linpix_segment(avhrrIn,lon,lat,lines,swath_width,tmppcs,
                                     tmpaid,covfilename):
    #import numpy.oldnumeric as Numeric
    import _satproj
    import area,pcs
    ...
    if not os.path.exists(covfilename):
        ...
        cov,info = readCoverage5km(covfilename)
...
##        if(x < 4500 and x >= 0 and y >= 0 and y < 4500): Should be 5010!!!/KG
        if(x < 5010 and x >= 0 and y >= 0 and y < 5010):
...
# -----------------------------------------------------
def match_calipso5km_avhrr5km(ctypefile,calipsoObj,avhrrGeoObj,avhrrObj,ctype,ctth,surft,ctypefile5km,calipso5kmObj,avhrr5kmGeoObj,avhrr5kmObj,ctype5km,ctth5km,surft5km):
    import numpy.oldnumeric as Numeric
    import time
    import string
    import numpy
    ...
    # --------------------------------------------------------------------
    cal5km,cap5km = get_calipso5km_avhrr5km_linpix(avhrr5kmGeoObj,lonCalipso5km,latCalipso5km)
    cal,cap = get_calipso5km_avhrr5km_linpix(avhrrGeoObj,lonCalipso,latCalipso)
    # --------------------------------------------------------------------
    idx_calipso = numpy.arange(ndim)
    idx_calipso_on_avhrr=numpy.repeat(idx_calipso,numpy.not_equal(cal,NODATA))
    
    lon_calipso = numpy.repeat(lonCalipso,numpy.not_equal(cal,NODATA))
    lat_calipso = numpy.repeat(latCalipso,numpy.not_equal(cal,NODATA))
    # Calipso line,pixel inside AVHRR swath:
    cal_on_avhrr = numpy.repeat(cal,numpy.not_equal(cal,NODATA))
    cap_on_avhrr = numpy.repeat(cap,numpy.not_equal(cal,NODATA))
    ...
    #timetup_end5km = time.gmtime(avhrr5km_sec1970_end)
    ...
    idx_match = select_calipso5km_inside_avhrr5km(calipsoObj,cal,dsec,secTup,sec_timeThr)

    retv.calipso.sec_1970 = numpy.repeat(calipsoObj.time[::,0] + dsec,idx_match)
    ...
    fd = open("data/%s_cloudtype_calipso_track2.txt"%(basename),"w")
    fd.writelines(ll)
    fd.close()
    ...
    fd = open("data/%s_cloudtype_calipso_track_excl.txt"%(basename),"w")
    fd.writelines(ll)
    fd.close()
...
def select_calipso5km_inside_avhrr5km(calipso5kmObj,cal5km,dsec,sec1970_start_end,sec_timeThr):
    ...
    idx_okay = numpy.logical_and(numpy.greater(\
        calipso5kmObj.time,sec1970_start - dsec - sec_timeThr),
                                   numpy.less(\
        calipso5kmObj.time,sec1970_end - dsec   + sec_timeThr))
        
    idx_match = numpy.not_equal(cal5km,NODATA)
...
# -----------------------------------------------------
def get_calipso5km(filename,filename5km):
    import _pypps_filters
    import numpy.oldnumeric as Numeric
    import numpy

    # Read CALIPSO Lidar (CALIOP) data:
    [calipso,calipso5km] = read_calipso5km(filename,filename5km)
    ...
    return calipso, calipso5km

# -----------------------------------------------------
def read_calipso5km(filename,filename5km):
    import _pyhl
    ...
    retv5 = Calipso5kmObject()

    c=a.getNode("/Longitude")
    retv5.longitude=c.data().astype('d')
    c=a.getNode("/Latitude")
    retv5.latitude=c.data().astype('d')
    c=a.getNode("/Profile_Time") # Internatiopnal Atomic Time (TAI) seconds from Jan 1, 1993
    retv5.time=c.data()
    c=a.getNode("/Profile_UTC_Time") # TAI time converted to UTC and stored in format yymmdd.fffffff    
    retv5.utc_time=c.data()

    c=a.getNode("/Feature_Classification_Flags")
    retv5.feature_classification_flags=c.data().astype('i')
    c=a.getNode("/Layer_Top_Altitude")
    retv5.cloud_top_profile=c.data()
    c=a.getNode("/Layer_Base_Altitude")
    retv5.cloud_base_profile=c.data()
    c=a.getNode("/Number_Layers_Found")
    retv5.number_of_layers_found=c.data()
    #c=a.getNode("/closest_calipso_cloud_fraction")
    #retv5.cloud_fraction=c.data()
    c=a.getNode("/Midlayer_Temperature")
    retv5.cloud_mid_temperature=c.data()

    c=a.getNode("/Day_Night_Flag")
    retv5.day_night_flag=c.data()
    c=a.getNode("/DEM_Surface_Elevation")
    retv5.elevation=c.data()
    c=a.getNode("/IGBP_Surface_Type")
    retv5.igbp=c.data()
    c=a.getNode("/NSIDC_Surface_Type")
    retv5.nsidc=c.data()
