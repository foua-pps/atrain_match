...
from cloudsat_calipso_avhrr_plot import drawCalClsatAvhrrPlotTimeDiff,\
    ...
    drawCalClsatCWCAvhrr1kmPlot, drawCalClsatCWCAvhrr5kmPlot #@UnresolvedImport
import pdb
        ...
        try:            
            if config.DEBUG is True:
                write_log('DEBUG', "Looking for avhrr file corresponding to %s: %s" % \
                          (str(cross), avhrr_finder.pattern(cross_time, cross_satellite)))
        except:
            pass
...
def get_calipso_matchups(calipso_files, cloudtype_file, avhrrGeoObj, avhrrObj, ctype, ctth, surft, avhrrAngObj):
    ...
    """
    calipso = reshapeCalipso(calipso_files,avhrrGeoObj, cloudtype_file)
...
def get_matchups_from_data(cross):
    ...
    PPS files.
    """
    import pps_io #@Reimport
    import epshdf #@Reimport
    import os #@Reimport
...
def get_matchups(cross, reprocess=False):
    ...
    if reprocess is False:
        ...
        try:
            ...
            write_log('INFO', "CloudSat Matchups read from previously processed data.")
        except IndexError:
            write_log('INFO', "No processed CloudSat match files found. Generating from source data.")
        try:
...
#    if process_mode_dnt in config.PLOT_MODES:
        ...
        if clsatObj == None:
            plotSatelliteTrajectory(calon,calat,trajectoryname,'eps')
            plotSatelliteTrajectory(avhrlon,avhrlat,trajectoryname+'avhrr','eps')
            pdb.set_trace()
            drawCalClsatAvhrrPlotTimeDiff(calat, None, caObj.diff_sec_1970, plotpath, basename, config.RESOLUTION)
            drawCalClsatGEOPROFAvhrrPlot(None, None, caObj.calipso, caObj.avhrr, elevation, data_ok,
            ...
            drawCalClsatAvhrrPlotSATZ(calat, None, caObj.avhrr.satz, plotpath, basename, config.RESOLUTION)
        else:                    
            if cloudsat_type=='GEOPROF': 
